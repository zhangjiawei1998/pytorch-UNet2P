...
def parse_args():
    ...
    parser.add_argument('--name', default='data_zebra_crossing_NestedUNet_woDS',
                        help='model name')
...
def main():
    ...
    for c in range(config['num_classes']):
        os.makedirs(os.path.join('data_zebra_crossing', config['name'], str(c)), exist_ok=True)
    with torch.no_grad():
        for input, target, meta in tqdm(val_loader, total=len(val_loader)):
            ...
            for i in range(len(output)):
                for c in range(config['num_classes']):
                    cv2.imwrite(os.path.join('data_zebra_crossing', config['name'], str(c), meta['img_id'][i] + '.jpg'),
